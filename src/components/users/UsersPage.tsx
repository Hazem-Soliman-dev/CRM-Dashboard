...
import { Plus, Search, Eye, Edit, Key, UserX, Mail, Download, Users, UserCheck, Shield, Clock, User } from 'lucide-react';
...
export const UsersPage: React.FC = () => {
  ...
  const loadData = useCallback(async () => {
    ...
    try {
      ...
      toast.error('Error', err.response?.data?.message || 'Failed to load users data');
    } finally {
      setLoading(false);
    }
  ...
  const getLastLoginStatus = (lastLogin: string) => {
    ...
    if (diffHours < 1) return { text: 'Online Now', color: 'text-green-600 dark:text-green-400' };
    if (diffHours < 24) return { text: `${Math.floor(diffHours)} hours ago`, color: 'text-blue-600 dark:text-blue-400' };
    const diffDays = Math.floor(diffHours / 24);
    return { text: `${diffDays} days ago`, color: 'text-gray-600 dark:text-gray-400' };
  };
  ...
  const handleAddUser = async (userData: any) => {
    ...
    try {
      ...
      await loadData();
      toast.success('User Added', 'New user has been created successfully. Temporary password has been generated.');
    } catch (error: any) {
  ...
  const handleToggleStatus = async (userId: string) => {
    if (!canPerformAction('users', 'update')) {
      toast.error('Access Denied', 'You do not have permission to modify user status.');
      return;
    }
  ...
  const handleResetPassword = (userId: string) => {
    toast.info('Password Reset', 'Password reset email has been sent to the user.');
  };
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between">
        <div>
          <h1 className="text-2xl font-bold text-gray-900 dark:text-white">User Management & Roles</h1>
          ...
          <CardContent className="p-6">
            <div className="flex items-center justify-between">
              <div>
                ...
                <p className="text-xs text-green-600 dark:text-green-400 mt-1">Online now: {onlineNow}</p>
              </div>
          ...
          <CardContent className="p-6">
            <div className="flex items-center justify-between">
              <div>
                ...
                <p className="text-xs text-gray-600 dark:text-gray-400 mt-1">Admin, Manager, Sales...</p>
              </div>
          ...
          <CardContent className="p-6">
            <div className="flex items-center justify-between">
              <div>
                ...
                <p className="text-xs text-green-600 dark:text-green-400 mt-1">Sarah Johnson</p>
              </div>
              <div className="p-3 bg-green-50 dark:bg-green-900/50 rounded-lg">
            ...
            <table className="w-full">
              ...
              <tbody className="bg-white dark:bg-gray-800 divide-y divide-gray-200 dark:divide-gray-700">
                {filteredUsers.map((user) => {
                  const loginStatus = user.lastLogin ? getLastLoginStatus(user.lastLogin) : { text: 'Never', color: 'text-gray-400' };
                    ...
                    <tr key={user.id} className="hover:bg-gray-50 dark:hover:bg-gray-700">
                      ...
                      <td className="px-6 py-4 whitespace-nowrap">
                        <div className="text-sm text-gray-900 dark:text-white">
                          <span className={loginStatus.color}>{loginStatus.text}</span>
                        </div>
                        {user.lastLogin && (
                        ...
                        <div className="flex space-x-2">
                          ...
                          <ActionGuard module="users" action="update">
                            <button 
                              onClick={(e) => {
                                e.preventDefault();
                                if (window.confirm(`Are you sure you want to ${user.status === 'Active' ? 'disable' : 'enable'} this user?`)) {
