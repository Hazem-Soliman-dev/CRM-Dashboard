...
import { Users, Building2, PhoneCall, Mail, MapPin, Plus, Edit, Trash2, UserCog } from 'lucide-react';
...
export const OwnersPage: React.FC = () => {
  ...
  const [statusFilter, setStatusFilter] = useState<'All' | Owner['status']>('All');
  ...
  const loadOwners = useCallback(async () => {
    try {
      ...
      showError('Failed to load owners', error.response?.data?.message || error.message);
    } finally {
      setLoading(false);
    }
  ...
  const handleCreateOwner = async (ownerData: any) => {
    if (!canPerformAction('owners', 'create')) {
      showError('Access Denied', 'You do not have permission to create owners.');
      return;
    }
    ...
    try {
      ...
      showError('Failed to create owner', error.response?.data?.message || error.message);
    }
  };

  const handleUpdateOwner = async (ownerId: number, ownerData: any) => {
    if (!canPerformAction('owners', 'update')) {
      showError('Access Denied', 'You do not have permission to update owners.');
      return;
    }
    ...
    try {
      ...
      showError('Failed to update owner', error.response?.data?.message || error.message);
    }
  };

  const handleDeleteOwner = async (ownerId: number) => {
    if (!canPerformAction('owners', 'delete')) {
      showError('Access Denied', 'You do not have permission to delete owners.');
      return;
    }
    ...
    try {
      ...
      await loadOwners();
      showSuccess('Owner Deleted', 'Owner has been deleted successfully.');
    } catch (error: any) {
      console.error('Error deleting owner:', error);
      showError('Failed to delete owner', error.response?.data?.message || error.message);
    }
  };
  ...
  const filteredOwners = useMemo(() => {
    return owners.filter((owner) => {
      const matchesSearch =
        ...
        (owner.primaryContact || '').toLowerCase().includes(search.toLowerCase());
  ...
  return (
    <div className="space-y-6">
      <div className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
        <div>
          <h1 className="text-2xl font-bold text-gray-900 dark:text-white">Property Owners</h1>
          <p className="text-gray-600 dark:text-gray-400">
            Manage institutional partners and ensure property portfolios stay active.
          </p>
        </div>
        ...
        <div className="flex flex-wrap items-center gap-3">
          ...
          <Select
            label="Status"
            value={statusFilter}
            onChange={(event) => setStatusFilter(event.target.value as 'All' | Owner['status'])}
          >
      ...
      <div className="grid grid-cols-1 gap-4 lg:grid-cols-2">
        {filteredOwners.map((owner) => (
          <Card key={owner.id} className="border border-gray-200 dark:border-gray-700">
            <CardHeader>
              <CardTitle className="flex items-center justify-between">
                <span className="text-lg font-semibold text-gray-900 dark:text-white">
                  {owner.companyName}
                </span>
                <span className={`text-sm font-medium ${statusStyles[owner.status]}`}>
                  {owner.status}
                </span>
              </CardTitle>
            </CardHeader>
            <CardContent className="space-y-3 text-sm text-gray-600 dark:text-gray-300">
              ...
              <div className="flex items-start gap-2">
                ...
                <span>{owner.locations.join(', ') || 'No locations listed'}</span>
              </div>
